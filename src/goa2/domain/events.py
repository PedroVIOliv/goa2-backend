"""
Game Event System - Phase 3 of Client-Readiness Roadmap.

Structured event stream so clients can animate actions, show logs,
and build replays. Events are ephemeral by-products of step resolution,
NOT part of GameState.
"""

from __future__ import annotations

from enum import StrEnum
from typing import Any

from pydantic import BaseModel, Field

from goa2.domain.hex import Hex


class GameEventType(StrEnum):
    # Movement
    UNIT_MOVED = "UNIT_MOVED"
    TOKEN_MOVED = "TOKEN_MOVED"
    UNIT_PLACED = "UNIT_PLACED"
    TOKEN_PLACED = "TOKEN_PLACED"
    BOARD_ENTITY_PLACED = "BOARD_ENTITY_PLACED"
    UNIT_PUSHED = "UNIT_PUSHED"
    TOKEN_PUSHED = "TOKEN_PUSHED"
    UNITS_SWAPPED = "UNITS_SWAPPED"
    # Combat
    COMBAT_RESOLVED = "COMBAT_RESOLVED"
    UNIT_DEFEATED = "UNIT_DEFEATED"
    UNIT_REMOVED = "UNIT_REMOVED"
    TOKEN_REMOVED = "TOKEN_REMOVED"
    BOARD_ENTITY_REMOVED = "BOARD_ENTITY_REMOVED"
    # Effects / Markers
    EFFECT_CREATED = "EFFECT_CREATED"
    MARKER_PLACED = "MARKER_PLACED"
    MARKER_REMOVED = "MARKER_REMOVED"
    # Economy
    GOLD_GAINED = "GOLD_GAINED"
    GOLD_LOST = "GOLD_LOST"
    ITEM_GAINED = "ITEM_GAINED"
    LIFE_COUNTER_CHANGED = "LIFE_COUNTER_CHANGED"
    # Cards
    CARD_RETRIEVED = "CARD_RETRIEVED"
    CARD_SELECTED_FOR_GUESS = "CARD_SELECTED_FOR_GUESS"
    GUESSED_CARD_REVEALED = "GUESSED_CARD_REVEALED"
    CARD_REVEALED = "CARD_REVEALED"
    SPELL_CAST = "SPELL_CAST"
    SPELL_REMOVED_FROM_SPELLBOOK = "SPELL_REMOVED_FROM_SPELLBOOK"
    SPELLBOOK_PREPARED = "SPELLBOOK_PREPARED"
    WISH_CAST_COUNT_CHANGED = "WISH_CAST_COUNT_CHANGED"
    # NebKher: two resolved cards traded turn slots (active effects preserved)
    RESOLVED_CARDS_SWAPPED = "RESOLVED_CARDS_SWAPPED"
    # Takahide: a card in play traded places with a card in the owner's deck
    DECK_CARD_SWAPPED = "DECK_CARD_SWAPPED"
    # Protection
    MINION_PROTECTED = "MINION_PROTECTED"
    # Turn flow
    TURN_ENDED = "TURN_ENDED"
    GAME_OVER = "GAME_OVER"
    TIMER_EXPIRED = "TIMER_EXPIRED"
    # Mines
    MINE_TRIGGERED = "MINE_TRIGGERED"
    # NebKher: a hero laughed diabolically as part of performing an action
    HERO_LAUGHED = "HERO_LAUGHED"

    TIE_BREAKER_FLIPPED = "TIE_BREAKER_FLIPPED"
    # Snorri: runes placed into rune slots
    RUNES_PLACED = "RUNES_PLACED"


def _hex_dict(h: Hex | None) -> dict[str, int] | None:
    """Serialize a Hex to a {q, r, s} dict for JSON compatibility."""
    if h is None:
        return None
    return {"q": h.q, "r": h.r, "s": h.s}


class GameEvent(BaseModel):
    """A single game event emitted by a step during resolution."""

    event_type: GameEventType
    actor_id: str | None = None
    target_id: str | None = None
    from_hex: dict[str, int] | None = None
    to_hex: dict[str, int] | None = None
    metadata: dict[str, Any] = Field(default_factory=dict)
