...
class CreateEffectStep(GameStep):
    ...
    def resolve(self, state: GameState, context: dict[str, Any]) -> StepResult:
        from goa2.engine.steps.markers import TOKEN_TYPE_OVERRIDE_KEY, substitution_applies
        ...
        if (
            self.is_token_effect
            and context.get(TOKEN_TYPE_OVERRIDE_KEY)
            and substitution_applies(state, context)
        ):
            return StepResult(is_finished=True)
