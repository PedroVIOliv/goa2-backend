...
class GameSetup:
    ...
    def create_game(
        ...
        print(f"[Setup] Game Created. Map: {len(board.tiles)} tiles. Players: {total_players}. Life: {counters}.")
        print(f"[Setup] Battle Zone: {active_zone_id}. Coin Favors: {state.tie_breaker_team.name}")
    ...
    def _setup_team(state: GameState, team_color: TeamColor, hero_names: List[str]):
        ...
        """
        available_spawns = [
            sp for sp in state.board.spawn_points 
    ...
    def _spawn_initial_minions(state: GameState, zone_id: str):
        ...
        for h in zone.hexes:
            ...
            if sp and sp.type == SpawnType.MINION:
                # Create Minion
