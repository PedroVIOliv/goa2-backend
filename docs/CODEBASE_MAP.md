...
> ⚠️ **STALE (as of 2026-07).** This map predates several structural changes and
> is kept only for the high-level architecture overview. Where it conflicts with
> the current code, the code wins. Known drift:
> - `engine/steps.py` is now an **11-file package** (`engine/steps/`:
>   base/cards/combat/movement/selection/utility/markers/reactions/phases/effects/__init__).
> - `engine/filters.py` / `engine/validation.py` have been **split** into multiple
>   `filters_*.py` modules (with a `filters.py` facade).
> - Hero **effect logic** lives in `src/goa2/scripts/<hero>_effects.py` (~25 heroes),
>   not just the 5 shown here. `data/heroes/` holds static Card/Hero data only.
> - The **`server/`** package (FastAPI REST + WebSocket) is the primary deliverable
>   and is not reflected below.
> - The `AnyStep`/`AnyFilter` serialization unions are **auto-derived** by reflection
>   in `engine/step_types.py`; they are not hand-edited.
>
> For corrected ground truth see `docs/MAINTAINABILIY_ROADMAP.md` §0 and `CLAUDE.md`.
> To fully refresh this map, re-run the Cartographer skill.
