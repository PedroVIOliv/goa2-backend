...
def test_phases_commit_and_revelation():
    ...
    )  # Should auto-transition through Revelation to Resolution
...
def test_rules_validate_target_geometry():
    ...
    assert (
        rules.validate_target(
            h1, h2, ActionType.ATTACK, state, range_val=3, requires_straight_line=True
        )
        is True
    )
...
def test_rules_validate_movement_obstacles():
    ...
    assert (
        rules.validate_movement_path(
            board=state.board, start=Hex(q=0, r=0, s=0), end=obs_hex, max_steps=1
        )
        is False
    )
    ...
    assert (
        rules.validate_movement_path(
            board=state.board, start=Hex(q=0, r=0, s=0), end=target, max_steps=2
        )
        is False
    )
