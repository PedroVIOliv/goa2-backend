...
from goa2.engine.steps import (
    ResolveCardStep, ResolveCardTextStep, MoveUnitStep, 
    AttackSequenceStep, LogMessageStep
)
...
def test_resolve_card_text_fallback_movement(fallback_state):
    ...
    hero.current_turn_card = Card(
        id="c1", name="Move", tier=CardTier.I, initiative=10,
        color=CardColor.RED,
        primary_action=ActionType.MOVEMENT, primary_action_value=3,
        effect_id="non_existent_effect", effect_text="Move 3",
        is_facedown=False
    )
...
def test_resolve_card_text_fallback_skill(fallback_state):
    hero = fallback_state.get_hero("A")
    hero.current_turn_card = Card(
        id="c1", name="Skill", tier=CardTier.I, initiative=10,
        color=CardColor.BLUE,
        primary_action=ActionType.SKILL, primary_action_value=None,
        effect_id="non_existent_effect", effect_text="Skill text",
        is_facedown=False
    )
...
def test_choose_secondary_hold(fallback_state):
    hero = fallback_state.get_hero("A")
    hero.current_turn_card = Card(
        id="c1", name="HoldTest", tier=CardTier.I, initiative=10,
        color=CardColor.GREEN,
        primary_action=ActionType.ATTACK, primary_action_value=4,
...
def test_choose_secondary_clear(fallback_state):
    hero = fallback_state.get_hero("A")
    hero.current_turn_card = Card(
        id="c1", name="ClearTest", tier=CardTier.I, initiative=10,
        color=CardColor.RED,
        primary_action=ActionType.ATTACK, primary_action_value=4,
...
def test_resolve_card_text_fallback_defense(fallback_state):
    hero = fallback_state.get_hero("A")
    hero.current_turn_card = Card(
        id="c1", name="Def", tier=CardTier.I, initiative=10,
        color=CardColor.BLUE,
        primary_action=ActionType.DEFENSE, primary_action_value=5,
        effect_id="non_existent_effect", effect_text="Def 5",
        is_facedown=False
    )
...
def test_choose_secondary_defense_active(fallback_state):
    # Testing Line 915-917 (Secondary DEFENSE - rare but handled)
    hero = fallback_state.get_hero("A")
    hero.current_turn_card = Card(
        id="c1", name="DefTest", tier=CardTier.I, initiative=10,
        color=CardColor.BLUE,
        primary_action=ActionType.SKILL, primary_action_value=None,
    ...
    push_steps(fallback_state, [step])
    process_resolution_stack(fallback_state)
    fallback_state.execution_stack[-1].pending_input = {"choice_id": "DEFENSE"}
    process_resolution_stack(fallback_state)
    assert len(fallback_state.execution_stack) == 0
