...
def test_ebb_and_flow_prevention(empty_state: GameState):
    state = empty_state
    arien_id = "hero_arien"
    ...
    # Add PREVENT_ACTION_REPEAT modifier
    from goa2.domain.models import Modifier, DurationType
    from goa2.domain.types import BoardEntityID, ModifierID

    mod = Modifier(
        id=ModifierID("mod_prevent"),
        source_id=BoardEntityID("source"),
        target_id=BoardEntityID(arien_id),
    ...
    check_step = next(s for s in steps if isinstance(s, CheckAdjacencyStep))
    check_step.resolve(state, state.execution_context)
    # Adjacency is TRUE so can_repeat=True
    assert state.execution_context["can_repeat"] is True
    ...
    # Result should be FINISHED (blocked by modifier)
    assert res.is_finished is True
    assert not res.new_steps

    # Since can_repeat is False, the second SelectStep should be skipped (if run in engine)
    # But here we inspect steps list
