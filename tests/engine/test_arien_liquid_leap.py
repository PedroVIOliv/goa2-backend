...
def test_liquid_leap_filters(leap_state):
    ...
    assert Hex(q=2,r=0,s=-2) in valid_hexes
    assert Hex(q=0,r=1,s=-1) in valid_hexes
    
    # invalid:
    assert Hex(q=1,r=0,s=-1) not in valid_hexes # Adj to empty spawn
    assert Hex(q=1,r=-1,s=0) not in valid_hexes # Is spawn
    assert Hex(q=2,r=-1,s=-1) not in valid_hexes # Adj to empty spawn
    assert Hex(q=0,r=0,s=0) not in valid_hexes # Occupied by self
