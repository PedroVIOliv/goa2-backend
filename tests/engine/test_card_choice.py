...
def choice_state():
    ...
    hero = create_test_hero(
        "A", TeamColor.RED, (ActionType.ATTACK, 4, {ActionType.MOVEMENT: 2, ActionType.HOLD: 0})
    )

    state = GameState(
        board=Board(), teams={TeamColor.RED: Team(color=TeamColor.RED, heroes=[hero], minions=[])}
    )
...
def test_resolve_card_no_primary():
    ...
    hero = create_test_hero("B", TeamColor.BLUE, (ActionType.SKILL, None, {ActionType.HOLD: 0}))
    hero.current_turn_card.is_facedown = True

    state = GameState(
        board=Board(), teams={TeamColor.BLUE: Team(color=TeamColor.BLUE, heroes=[hero], minions=[])}
    )
    state.move_unit(hero.id, Hex(q=0, r=0, s=0))
