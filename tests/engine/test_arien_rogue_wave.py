...
def test_rogue_wave_range_buff(rogue_wave_state):
    """Test that range stat is computed with buffs."""
    from goa2.domain.models import StatType
    from goa2.domain.models.modifier import Modifier, DurationType

    # Add +1 range modifier to Arien
    rogue_wave_state.active_modifiers.append(
        Modifier(
            id="range_buff",
            source_id="test",
            target_id="arien",
    ...
    # Add an enemy at range 3
    rogue_wave_state.board.tiles[Hex(q=0, r=3, s=-3)] = rogue_wave_state.board.tiles[
        Hex(q=0, r=0, s=0)
    ].__class__(hex=Hex(q=0, r=3, s=-3))
    far_enemy = Hero(id="far_enemy", name="Far", team=TeamColor.BLUE, deck=[], level=1)
    rogue_wave_state.teams[TeamColor.BLUE].heroes.append(far_enemy)
    rogue_wave_state.place_entity("far_enemy", Hex(q=0, r=3, s=-3))
    ...
    # 2. Target selection - far_enemy should be valid (range 3 with +1 buff)
    req = process_resolution_stack(rogue_wave_state)
    assert req["type"] == "SELECT_UNIT"
    assert "far_enemy" in req["valid_options"], (
        "Range buff should allow targeting at range 3"
    )
