...
from goa2.engine.steps import ResolveDisplacementStep, StepResult, PlaceUnitStep
...
def displacement_state():
    board = Board()
    # Zone 1
    z1 = Zone(id="z1", name="Battle", hexes=[
    ...
    m_block = Minion(id="m_block", name="Blocker2", type=MinionType.MELEE, team=TeamColor.RED) # Blocks neighbor
...
def test_displacement_prompt(displacement_state):
    ...
    assert Hex(q=1, r=0, s=-1) in req["valid_hexes"]
    assert Hex(q=1, r=-1, s=0) in req["valid_hexes"]
...
def test_displacement_multi_unit_selection(displacement_state):
    ...
    displacement_state.execution_stack[-1].pending_input = {"selected_unit_id": "m_disp_2"}
