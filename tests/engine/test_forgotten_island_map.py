...
MAP_PATH = os.path.join(
    os.path.dirname(__file__), "..", "..", "src", "goa2", "data", "maps", "forgotten_island.json"
)
...
class TestSpawnPoints:
    ...
    def test_jungles_have_no_spawn_points(self, board, zones_by_label):
        ...
        assert (
            len(red_jungle_spawns) == 0
        ), f"RedJungle should have no spawn points, but has {len(red_jungle_spawns)}"
        assert (
            len(blue_jungle_spawns) == 0
        ), f"BlueJungle should have no spawn points, but has {len(blue_jungle_spawns)}"
