...
def test_attack_legacy_fallback(base_state):
    ...
    assert (
        rules.validate_attack_target(
            attacker_pos=Hex(q=0, r=0, s=0), target_pos=Hex(q=1, r=0, s=-1), range_val=1
        )
        is True
    )
    ...
    assert (
        rules.validate_attack_target(
            attacker_pos=Hex(q=0, r=0, s=0), target_pos=Hex(q=2, r=0, s=-2), range_val=1
        )
        is False
    )
