...
def test_hex_filter_occupied(filter_state):
    ...
    step = SelectStep(
        target_type="HEX",
        prompt="Test",
        filters=[RangeFilter(max_range=1), ObstacleFilter(is_occupied=False)],
    )
    res = step.resolve(filter_state, {})
    valid = res.input_request["valid_options"]

    target_hex = Hex(q=1, r=-1, s=0)
    assert target_hex.model_dump() in valid
    assert Hex(q=0, r=0, s=0).model_dump() not in valid  # Occupied
    assert Hex(q=1, r=0, s=-1).model_dump() not in valid  # Occupied
...
def test_auto_select(filter_state):
    ...
    step = SelectStep(
        target_type="UNIT",
        prompt="Test",
        filters=[UnitTypeFilter(unit_type="MINION"), TeamFilter(relation="ENEMY")],
