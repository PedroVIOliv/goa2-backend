...
def rules_state():
    ...
    zones = {"RedBase": Zone(id="RedBase", hexes={red_base_hex})}
    board.zones = zones
    ...
    state = GameState(
        board=board,
        teams={TeamColor.RED: Team(color=TeamColor.RED, heroes=[h1])},
        unit_locations={"h1": red_base_hex},
...
def test_validate_movement_path_edge_cases(rules_state):
    ...
    locs = rules_state.unit_locations
    start = Hex(q=0, r=0, s=0)
    
    # start == end
    assert validate_movement_path(board, locs, start, start, 5) is False
    
    # end in unit_locations.values()
    h2_hex = Hex(q=1, r=0, s=-1)
    locs["h2"] = h2_hex
    assert validate_movement_path(board, locs, start, h2_hex, 5) is False
    ...
    assert validate_movement_path(board, locs, start, far_hex, 10) is False
...
def test_is_immune_success(rules_state):
    ...
    m_heavy = Minion(id="m_heavy", name="Heavy", type=MinionType.HEAVY, team=TeamColor.RED, is_heavy=True)
    rules_state.teams[TeamColor.RED].minions.append(m_heavy)
    rules_state.unit_locations["m_heavy"] = Hex(q=0, r=0, s=0)
    ...
    rules_state.unit_locations["m_friend"] = Hex(q=1, r=0, s=-1) # RedBase neighbor
    
    # RedBase zone includes both hexes
    rules_state.board.zones["RedBase"].hexes.add(Hex(q=1, r=0, s=-1))
    
    assert is_immune(m_heavy, rules_state) is True
    
    # If friend is NOT in the zone
    rules_state.unit_locations["m_friend"] = Hex(q=10, r=10, s=-20)
    assert is_immune(m_heavy, rules_state) is False
...
def test_validate_movement_path_ignore_obstacles(rules_state):
    ...
    # Normally False
    assert validate_movement_path(board, locs, start, end, 5, ignore_obstacles=False) is False
    # With ignore_obstacles=True, it should pass
    assert validate_movement_path(board, locs, start, end, 5, ignore_obstacles=True) is True
...
def test_validate_movement_path_obstacle_end(rules_state):
    ...
    # Line 34: end in unit_locations.values()
    locs["other_unit"] = end
    assert validate_movement_path(board, locs, start, end, 5) is False
    del locs["other_unit"]
    
    # Line 31: End tile is obstacle
    board.tiles[end] = Tile(hex=end, is_terrain=True) # Terrain is obstacle
    assert validate_movement_path(board, locs, start, end, 5) is False
    
    # Line 46: Neighbor is obstacle but not end
    mid = Hex(q=1, r=0, s=-1)
    ...
    # Path blocked by mid
    assert validate_movement_path(board, locs, start, end_ok, 5) is False
...
def test_validate_movement_path_dist_limit(rules_state):
    ...
    assert validate_movement_path(board, locs, start, end, 1) is False
...
def test_validate_attack_target_legacy_straight_line():
    ...
    assert validate_attack_target({}, attacker_pos, target_pos, 5, requires_straight_line=True) is False
    
    # Out of range (fallback path)
    assert validate_attack_target({}, attacker_pos, target_pos, 1, requires_straight_line=False) is False
