...
def test_validate_movement_path_edge_cases(rules_state):
    ...
    # start == end
    assert validate_movement_path(board=board, start=start, end=start, max_steps=5) is False

    # end is occupied (using place_entity to simulate)
    h2_hex = Hex(q=1, r=0, s=-1)
    rules_state.place_entity("h2", h2_hex)

    assert validate_movement_path(board=board, start=start, end=h2_hex, max_steps=5) is False
    ...
    assert validate_movement_path(board=board, start=start, end=far_hex, max_steps=10) is False


def test_is_immune_edge_cases(rules_state):
    # Heavy minion check
    m_heavy = Minion(
        id="m_heavy", name="Heavy", type=MinionType.HEAVY, team=TeamColor.RED, is_heavy=True
    )
...
def test_validate_movement_path_ignore_obstacles(rules_state):
    ...
    assert (
        validate_movement_path(
            board=board, start=start, end=end, max_steps=5, ignore_obstacles=False
        )
        is False
    )
    ...
    assert (
        validate_movement_path(
            board=board, start=start, end=end, max_steps=5, ignore_obstacles=True
        )
        is True
    )
...
def test_validate_movement_path_obstacle_end(rules_state):
    ...
    assert validate_movement_path(board=board, start=start, end=end, max_steps=5) is False
    ...
    # Path blocked by mid
    assert validate_movement_path(board=board, start=start, end=end_ok, max_steps=5) is False
...
def test_validate_movement_path_dist_limit(rules_state):
    ...
    assert validate_movement_path(board=board, start=start, end=end, max_steps=1) is False
