...
def test_level_8_does_not_create_pending_upgrade():
    """
    Reaching level 8 should NOT create a pending upgrade.
    Ultimate cards auto-activate at level 8 without player choice.
    """
    from goa2.domain.board import Board
    ...
    ultimate = Card(
        ...
        primary_action_value=None,
        effect_id="test_ult",
        effect_text="Test ultimate ability",
    )
    ultimate.state = CardState.PASSIVE

    # Hero at level 7 with exactly 7 gold (cost for level 8)
    h1 = Hero(
        id="h1",
        name="H1",
        team=TeamColor.RED,
    ...
    # Should NOT have pending upgrades (ultimate auto-activates)
    assert "h1" not in state.pending_upgrades

    # Phase should still transition to PLANNING (no upgrade choices needed)
    # Note: Since there are no pending upgrades, phase goes to PLANNING via RoundReset
    assert state.phase == GamePhase.PLANNING


def test_level_7_to_8_with_other_upgrades():
    """
    If a hero levels from 5 to 8 in one round, levels 6 and 7 should
    create pending upgrades, but level 8 should not.
    """
    from goa2.domain.board import Board
    ...
    # Hero at level 5 with enough gold to reach level 8
    # Costs: 6->5, 7->6, 8->7 = 18 gold total from level 5
    h1 = Hero(
        id="h1",
        name="H1",
        team=TeamColor.RED,
    ...
    # Should have 2 pending upgrades (levels 6 and 7, NOT level 8)
    assert state.pending_upgrades.get("h1") == 2


def test_no_pity_coin_when_only_ultimate_unlocked():
    """
    If a hero only unlocks their ultimate (level 7 -> 8), they should
    NOT receive a pity coin (they did level up).
    """
    from goa2.domain.board import Board
    ...
    # Hero at level 7 with exactly 7 gold
    h1 = Hero(
        id="h1",
        name="H1",
        team=TeamColor.RED,
