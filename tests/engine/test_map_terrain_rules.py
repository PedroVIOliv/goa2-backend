...
def test_terrain_outside_zone():
    ...
    map_data = {
        "zone_definitions": [{"id": "zone1", "label": "Zone 1"}],
        "hex_map": [
...
def test_missing_hex_is_terrain():
    ...
    board = Board(tiles={Hex(q=0, r=0, s=0): Tile(hex=Hex(q=0, r=0, s=0))})
...
def test_movement_blocked_by_void():
    ...
    assert (
        rules.validate_movement_path(
            board=board, start=Hex(q=0, r=0, s=0), end=neighbor, max_steps=1
        )
        is False
    )
