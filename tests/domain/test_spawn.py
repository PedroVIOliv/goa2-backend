...
def test_valid_hero_spawn():
    sp = SpawnPoint(location=Hex(q=0, r=0, s=0), team=TeamColor.BLUE, type=SpawnType.HERO)
...
def test_invalid_minion_spawn_missing_type():
    with pytest.raises(ValueError, match="Minion spawn point must specify minion_type"):
        SpawnPoint(
            location=Hex(q=0, r=0, s=0), team=TeamColor.RED, type=SpawnType.MINION, minion_type=None
        )
