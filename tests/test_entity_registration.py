...
from goa2.domain.models import Hero, MinionType, Team, TeamColor, TokenType
...
def test_register_hero_auto_sorts_to_team(reg_state):
    ...
    """
    hero = Hero(id=HeroID("hero_test"), name="Test Hero", deck=[], team=TeamColor.RED)
...
def test_register_token_goes_to_misc(reg_state):
    ...
    """
    token = EntityFactory.create_token(reg_state, TokenType.SMOKE_BOMB, "Trap")
...
def test_duplicate_registration_raises_error(reg_state):
    ...
    imposter = EntityFactory.create_token(reg_state, TokenType.SMOKE_BOMB, "Imposter")
